...
(define (drop tagged-value)
  ; (display (list 'drop tagged-value)) (newline)
...
(define (install-rational-package)
  ...
  (add-raise-func 'rational 'real (lambda (r) (div (numer r) (denom r))))
...
(define (install-polynomial-package)
  ...
  (define (equ-terms? t1 t2)
    (cond
      [(empty-termlist? t1) (empty-termlist? t2)]
      [(empty-termlist? t2) #f]
      [else
        (if (equ? (first-term t1) (first-term t2))
  ...
  (define (gcd-terms a b)
    (if (empty-termlist? b)
      (let [(divisor (apply gcd (map coeff a)))]
        (map (lambda (c) (make-term (order c) (/ (coeff c) divisor))) a))
