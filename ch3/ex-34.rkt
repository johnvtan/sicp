...
; based on the way multiplier is defined, values won't be propagated in this
; direction because the multiplier will only set its value if 2 of the 3 constraints
; have a value. Because both the multiplier's inputs are the same connector here,
; we are never able to use the squarer by setting the squared value. 
(set-value! b 64 'user)