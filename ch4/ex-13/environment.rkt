...
(define (make-unbound! var env)
  (let [(frame (first-frame env))]
    (let [(alist (map cons (frame-variables frame) (frame-values frame)))]
      (let [(filtered (filter (lambda (elem) (not (eq? (car elem) var))) alist))]
        (let [(filtered-vars (map car filtered))
              (filtered-vals (map cdr filtered))]
          
          (set-car! frame filtered-vars)
          (set-cdr! frame filtered-vals)
          (display (list 'make-unbound-after var filtered frame)) (newline))))))
...
(#%provide define-variable! set-variable-value! extend-environment lookup-variable-value
          the-global-environment setup-environment make-unbound!)