...
(define (make-procedure parameters body env)
  ;(display (list 'make-procedure (scan-out-defines body))) (newline)
  (list 'procedure parameters (scan-out-defines body) env))
(define (compound-procedure? p)
  (tagged-list? p 'procedure))

(define (scan-out-defines body)
  (let [(defines (filter (lambda (expr) (definition? expr)) body))
        (others (filter (lambda (expr) (not (definition? expr))) body))]
    (if (null? defines)
      body
      ; I forget why we need to enclose this in a list (i.e., apply this immediately)?
      (list
        (create-let-sets (map definition-variable defines) (map definition-value defines) others)))))

(define (create-let-sets vars vals body)
  (if (null? vars)
    (error "bad create let sets")
    (make-let
      (map
        (lambda (var) (list var (make-quoted '*unassigned*))) vars)
      (append (map make-assignment vars vals) body))))
...
(define (letrec->let expr)
  (let [(bindings (letrec-bindings expr))]
    (if (null? bindings)
      (error "no bindings in letrect expr" expr)
      (create-let-sets (map car bindings) (map cadr bindings) (letrec-body expr)))))
