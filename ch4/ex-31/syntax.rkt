...
(define (lambda-parameter-eager? p) (not (pair? p)))
(define (lambda-parameter-lazy? p)
  (and (pair? p) (eq? (cadr p) 'lazy)))
(define (lambda-parameter-lazy-memo? p)
  (and (pair? p) (eq? (cadr p) 'lazy-memo)))
...
(define (unless-expected exp)
  (cadddr exp))

(define (unless->if exp)
  (make-if (unless-condition exp) (unless-exceptional exp) (unless-expected exp)))
...
(#%provide self-evaluating? variable?
  ...
  delay-it-memo memo-thunk?
  evaluated-thunk? thunk-value)