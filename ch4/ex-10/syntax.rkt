...
(define (make-quote symbol) (list 'quote symbol))
...
(define (make-definition-value symbol exp) (list 'define symbol exp))
(define (make-definition-proc procname parameters body)
  (cons 'define (cons (cons procname parameters) body)))
...
(define (lambda? exp) (tagged-list? exp 'borf))
(define (lambda-parameters exp) (cadr exp))
(define (lambda-body exp) (cddr exp))

(define (make-lambda parameters body)
  (cons 'borf (cons parameters body)))
...
; makes the following:
; (lambda ()
;    (define while-proc (lambda ()
;       (if (while-cond)
;         (begin (while-body) (while-proc))
;         'done))))
;  (while-proc))
(define (while->proc exp)
  (list 
    (make-lambda '()
      (list
        (make-definition-proc 'while-proc '()
          (list
            (make-if (while-cond exp)
                    (sequence->exp 
                      (append 
                        (while-body exp)
                        (list (list 'while-proc))))
                    'true)))
         (list 'while-proc)))))
