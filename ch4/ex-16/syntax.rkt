...
(define (scan-out-defines body)
  (let [(defines (filter (lambda (expr) (definition? expr)) body))
    ...
    (display (list 'scan-out-defines defines others)) (newline)
    (if (null? defines)
      body
      (list
        (make-let 
          (map 
            (lambda (define-expr) 
              (list (definition-variable define-expr) (make-quoted '*unassigned*))) defines)
          (append
            (map
              (lambda (define-expr)
                (make-assignment (definition-variable define-expr) (definition-value define-expr)))
              defines)
            others))))))
