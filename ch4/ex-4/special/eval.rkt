...
(define (myeval-and exp env)
  (cond
    [(null? exp) #t]
    [(myeval (and-first exp) env) (myeval-and (and-rest exp) env)]
    [else #f]))

(define (myeval-or exp env)
  (cond
    [(null? exp) #f]
    [(myeval (or-first exp) env) #t]
    [else (myeval-or (or-rest exp) env)]))
...
(define (myeval exp env)
  ...
  (cond
    ...
    [(and? exp) (myeval-and exp env)]
    [(or? exp) (myeval-or exp env)]
